...
func multiply(i int, inputs map[int]*big.Int , level_vec map[int]*big.Int, mutex *sync.Mutex, wg *sync.WaitGroup) {
    defer wg.Done()
    var content *big.Int
    if i+1 == len(inputs) {
        content = inputs[i]
...
func divide(index int, next_level map[int]*big.Int, current_level map[int]*big.Int, output_level map[int]*big.Int, mutex *sync.Mutex, wg *sync.WaitGroup) {
    defer wg.Done()
    sq := new(big.Int)
